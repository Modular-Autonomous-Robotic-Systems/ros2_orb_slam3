...
VisualSlamNode::on_activate(const rclcpp_lifecycle::State &previous_state) {
    ...
    mpSlam = std::unique_ptr<Slam>(
        new MonoORBSLAM3(this->get_logger(), mpSettingsFilePath,
                         mpVocabFilePath, "monocular-only"));
...
void VisualSlamNode::GrabImage(const ImageMsg::SharedPtr msg) {
    ...
    if (!img.empty()) {
        ...
        // from world frame to camera frame. Thus, Tcw * Xw would be equivalent
        // to Xw to Xc. To get the position of the camera with respect to slam
        // origin, the transformation needs to be inverted to obtain camera
        // position in world reference The following link documents an issue
        // related to this in ORBSLAM2 repo
        // https://github.com/raulmur/ORB_SLAM2/issues/226
        // One may also refer to the following link to check the implementation
        // of of inverse for Sophus::SE3f
        mpTwc = tcw.inverse();
