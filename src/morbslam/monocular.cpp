...
void MonoMORBSLAM::InitialiseSlam(
    ...
    if (readYAMLFile(configFilePath, slamConfig)) {
        ...
        if (slamConfig["Slam.SettingsFilePath"].IsDefined()) {
            settingsFilePath =
                slamConfig["Slam.SettingsFilePath"].as<std::string>();
            RCLCPP_INFO(mpLogger, "got settings file path: %s",
                        settingsFilePath);
        ...
        mpMORBSLAM = std::make_unique<MORB_SLAM::System>(
            vocabFilePath, settingsFilePath, cameraType);
    }
}

bool MonoMORBSLAM::TrackMonocular(Frame& frame, Sophus::SE3f& tcw) {
    // tcw =
    MORB_SLAM::MonoPacket packet =
        mpMORBSLAM->TrackMonocular(frame.getImage(), frame.getTimestampSec());
    // tcw = *(packet.pose);  // superseded 2026-08-23, dereferenced an empty
    // optional whenever MORB_SLAM produced no pose for the frame
    if (!packet.pose) {
        return false;
    }
    tcw = *(packet.pose);
