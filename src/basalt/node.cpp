...
BasaltSLAMNode::BasaltSLAMNode() : SlamNode("basalt_slam_node") {
    ...
    this->declare_parameter<std::string>("calibration_file_path", "", calib_file_path_desc);
    ...
    this->declare_parameter<std::string>("configuration_file_path", "", config_file_path_desc);
...
CallbackReturn BasaltSLAMNode::on_configure(const rclcpp_lifecycle::State &previous_state) {
    mpCameraTopicName = this->get_parameter("camera_topic_name").as_string();
    mpCalibrationFilePath = this->get_parameter("calibration_file_path").as_string();
    mpConfigurationFilePath = this->get_parameter("configuration_file_path").as_string();

    RCLCPP_INFO(this->get_logger(), "Camera Topic Name: %s", mpCameraTopicName.c_str());
    RCLCPP_INFO(this->get_logger(), "Calibration File Path: %s", mpCalibrationFilePath.c_str());
    RCLCPP_INFO(this->get_logger(), "Configuration File Path: %s", mpConfigurationFilePath.c_str());

    return CallbackReturn::SUCCESS;
}

CallbackReturn BasaltSLAMNode::on_activate(const rclcpp_lifecycle::State &previous_state) {
    RCLCPP_INFO(this->get_logger(), "Creating SLAM Object in Basalt Slam Node");

    mpSlam = std::make_unique<BasaltSLAM>(this->get_logger(), mpConfigurationFilePath, mpCalibrationFilePath, "monocular-only");
...
CallbackReturn BasaltSLAMNode::on_deactivate(const rclcpp_lifecycle::State &previous_state) {
...
CallbackReturn BasaltSLAMNode::on_cleanup(const rclcpp_lifecycle::State &previous_state) {
    return CallbackReturn::SUCCESS;
}
...
void BasaltSLAMNode::PublishFrame() {
    ...
    cv_bridge::CvImage(std_msgs::msg::Header(), sensor_msgs::image_encodings::BGR8, mpCurrentFrame.getImage().clone()).toImageMsg(*img_msg_ptr);
