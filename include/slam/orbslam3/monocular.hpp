...
class VisualSlamNode : public SlamNode {
  ...
  private:
...
#endif
#ifdef USE_MORBSLAM
    MORB_SLAM::TrackingState mpState =
        MORB_SLAM::TrackingState::SYSTEM_NOT_READY;
#endif
    ...
    rclcpp::Service<custom_interfaces::srv::StartupSlam>::SharedPtr
        mpSlamStartupService;
    ...
    void PublishFrame();
#ifdef USE_ORBSLAM3
    void PublishMapPointsCallback(std::vector<ORB_SLAM3::MapPoint *> &mapPoints,
                                  const Sophus::SE3<float> &tcw);
    void MapPointsToPointCloud(std::vector<ORB_SLAM3::MapPoint *> &mapPoints,
                               const Sophus::SE3<float> &tcw,
                               sensor_msgs::msg::PointCloud2 &cloud);
    void SophusToGeometryMsgTransform(const Sophus::SE3<float> &se3,
